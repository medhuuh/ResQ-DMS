    ...
    return (
        <div className="p-6">
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h2 className="text-3xl font-bold text-white">{t('nav.camps')}</h2>
                    <p className="text-gray-400">{t('camp.manageDesc')}</p>
                </div>
                {!viewOnly && (
                    <Link to="new" className="px-4 py-2 bg-primary text-white font-bold rounded-xl flex items-center gap-2 hover:bg-lime-700 transition shadow-lg shadow-lime-500/30">
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-black/20 border-b border-white/10 text-xs text-gray-400 uppercase tracking-wider">
                            ...
                            {showActions && <th className="p-4 font-medium">{t('actions')}</th>}
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-white/10">
                        {camps.map((camp) => (
                            <tr key={camp.id} className="hover:bg-white/5 transition">
                                ...
                                <td className="p-4 text-gray-400 flex items-center gap-1"><MapPin className="w-4 h-4" /> {camp.location}</td>
                                <td className="p-4">
                                ...
                                <td className="p-4">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-xs font-bold text-gray-300">{Math.round((camp.occupied / camp.capacity) * 100)}%</span>
                                    </div>
                                    <p className="text-xs text-gray-400 mt-1">{camp.occupied} / {camp.capacity} {t('occupied')}</p>
                                </td>
                                <td className="p-4">
                                    <span className="px-2 py-1 bg-green-500/20 text-green-400 border border-green-500/30 text-xs font-bold rounded-lg">{camp.status}</span>
                                </td>
                                {showActions && (
                                    <td className="p-4">
                                        <button
